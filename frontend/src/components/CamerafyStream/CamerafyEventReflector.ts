...
/** 12/10/2020 7:03:03 PM */
/* eslint-disable */


import { UUID4 } from '@/api/utils/utils';

export interface ICamerafyEvent
{
	id   : string;
	data : Uint8Array;
}

export enum CamerafySessionDataType
{
	EventResponse,
	SessionEvent
};

export enum CamerafyResponseStatus
{
	ok,
	error
};

export interface CamerafyResponseData
{
	status : CamerafyResponseStatus;
	result : any;
};

export interface ICamerafySessionData
{
	type   				: CamerafySessionDataType;
	responseRefId? 		: string;
	responseData? 		: CamerafyResponseData;
	sessionEventName?	: string;
	sessionEventArgs?	: Array<any>;
};
...
class CamerafyEventReflector
{
	constructor()
	{
	}

	public deserializeSessionData(InUint8Arr : Uint8Array) : ICamerafySessionData
	{
			const name = DecodeString(InUint8Arr, 0, 32);

			if(name == "00000000000000000000000000000000")
			{
				const data = new DataView(InUint8Arr.buffer);
				let offset = 32;
				let len = 0;
				...
				return { type: CamerafySessionDataType.EventResponse, responseRefId: messageId, responseData: JSON.parse(returnJson) };
			}

			return { type: CamerafySessionDataType.SessionEvent, sessionEventName: name, sessionEventArgs: eval(`__${name}`)(InUint8Arr) };


			/* Server side events argument parser */
			function __f0d485210b84b5c175f2ede07e65c746(InUint8Arr : Uint8Array)
			{
				let Args = new Array<any>();
				return Args;
			}
			
			function __4c1077f64c1b2ab1e77bea76a42ac158(InUint8Arr : Uint8Array)
			{
				let Args = new Array<any>();
				...
				/* sdp_mlineindex */
				let arg1 = [];
				arg1 = data.getInt32(offset, true); offset += 4;
				Args.push(arg1);
			
				/* sdp_mid */
				let arg2 = [];
				len = data.getInt32(offset, true); offset += 4;
				arg2 = DecodeString(InUint8Arr, offset, len); offset += len;
				Args.push(arg2);
			...
			function __7b0d19c41952df10fd16b07c52cb2dda(InUint8Arr : Uint8Array)
			{
				let Args = new Array<any>();
				let data = new DataView(InUint8Arr.buffer);
				let offset = 32; // skip first bytes, as they hold the method address
	...
	get User()
	{
		return {
			User:
			{
				NewIceCandidateReady: (candidate : string, sdp_mlineindex : number, sdp_mid : string) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + (4 + candidate.length) + 4 + (4 + sdp_mid.length));
					...
					return {id: messageId, data: new Uint8Array(OpCode) };
				},

				ClientRtcOffer: (sdp : string) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + (4 + sdp.length));
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
					...
					offset += EncodeString(messageId, OpCode, offset, false);
					offset += EncodeString("d29657f85b0e48a2435865ada82f278b", OpCode, offset, false);
				...
				Disconnect: () : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68);
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				RequestCameraSnapshot: (width : number, height : number, format : string) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4 + 4 + (4 + format.length));
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
	...
	get Input()
	{
		return {
			RemoteInputModule:
			{
				ProcessRemoteKeyboardInput: (IsKeyDown : boolean, IsRepeat : boolean, KeyCode : any, Char : string) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 1 + 1 + 1 + 2);
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				ProcessRemoteMouseInput: (Xpos : number, Ypos : number, Button : any) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 2 + 2 + 1);
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				ProcessRemoteMouseWheelInput: (ScrollX : number, ScrollY : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				ProcessRemoteTouchInput: (NumTouches : number, TouchId : Array<any>, Phase : Array<any>, XPos : Array<any>, YPos : Array<any>, Force : Array<any>) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4 + 	(4 + (TouchId.length * 4)) + 	(4 + Phase.length) + 	(4 + (XPos.length * 2)) + 	(4 + (YPos.length * 2)) + 	(4 + (Force.length * 4)));
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
	...
	get Camera()
	{
		return {
			CameraController:
			{
				SetMaxLinearSpeed: (InMaxLinearSpeed : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				SetMaxAngularSpeed: (InMaxAngularSpeed : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
				...
				SetInvertControls: (InState : boolean) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 1);
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				SetLinearDamping: (InDamping : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
				...
				SetAngularDamping: (InDamping : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
				...
				SetCameraTarget: (InX : number, InY : number, InZ : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4 + 4 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				SetCameraTargetByName: (InTargetName : string) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + (4 + InTargetName.length));
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
				...
				SetFreeBehaviour: (Weight : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
				...
				SetOrbitBehaviour: (Weight : number) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + 4);
					let data = new DataView(OpCode);
					let offset = 0;
	...
	get Application()
	{
		return {
			Session:
			{
				Connect: (InUserId : string) : ICamerafyEvent =>
				{
					let OpCode = new ArrayBuffer(68 + (4 + InUserId.length));
					let data = new DataView(OpCode);
					let offset = 0;
					let n = 0;
					let N = 1;
					...
					return {id: "" /* AWAITS NO RESPONSE */, data: new Uint8Array(OpCode) };
				},
...
/* Server side events */
export const ClientServerWebrtcReadyServerEvent = "f0d485210b84b5c175f2ede07e65c746";
export const NewIceCandidateReadyServerServerEvent = "4c1077f64c1b2ab1e77bea76a42ac158";
...
export const ServerRtcAnswerServerEvent = "7b0d19c41952df10fd16b07c52cb2dda";
...
const EventReflector = new CamerafyEventReflector();
export default EventReflector;
